...
namespace Phonebook.Data.Tests
{
	...
	[TestClass]
	public class BaseRespositoryTests
	{
		protected IPhonebookContext phonebookContext;
		protected IQueryable<User> _users;
		protected IQueryable<Contact> _contacts;
		protected IQueryable<ContactNumber> _contactNumbers;
		...
		public void InitialiseTests()
		{
			...
			contactNumberSet.Setup(x => x.Remove(It.IsAny<ContactNumber>())).Returns<ContactNumber>(x => { if (contactNumberList.Remove(x)) return x; else return null; });
			...
			contactSet.Setup(x => x.Remove(It.IsAny<Contact>())).Returns<Contact>(x => { if (contactList.Remove(x)) return x; else return null; });
			...
			userSet.Setup(x => x.Remove(It.IsAny<User>())).Returns<User>(x => { if (userList.Remove(x)) return x; else return null; });
